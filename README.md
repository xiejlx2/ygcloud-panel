...
一个多租户的服务器管理面板。管理员可接入一个或多个上游云账户的 OpenAPI Key，统一管理各账户中的服务器资源，再按服务器维度把机器分配给下游客户；客户登录后只能查看和操作分配给自己的服务器。

- 管理员可添加多个命名云账户，每个账户独立保存和校验 OpenAPI Key。
- 服务器会记录来源云账户，实时操作自动使用对应账户的 Key。
...
- **凭据加密**：所有云账户 Key 均经 AES-256-GCM 落库；任何前端接口都不返回明文，仅展示末 4 位。
...
1. **接入配置** → 添加一个或多个云账户 → 分别填入 OpenAPI Key → 保存并校验。
