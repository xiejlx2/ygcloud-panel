...
// 各渠道配置攻略。example 会以等宽代码样式单独高亮展示。
const GUIDES: Record<
  GuideTab,
...
> = {
  ...
  dingtalk: {
    ...
    steps: [
      ...
      "安全设置勾选「自定义关键词」，关键词填「服务器」（本面板消息都以【服务器控制台】开头，含此词即可通过校验）。",
      "复制 Webhook 地址。",
  ...
  feishu: {
    ...
    steps: [
      ...
      "安全设置可留空，或勾「自定义关键词」填「服务器」（本面板暂不支持签名校验）。",
      "复制 Webhook 地址。",
...
export default function AdminNotifyPage() {
  ...
  const { data, mutate, isLoading } = useSWR<NotifyInfo>("/api/admin/notify", api);
  ...
  async function post(body: Record<string, unknown>, okMsg: string, tag: string) {
    setBusy(tag);
    try {
      await api("/api/admin/notify", { method: "POST", body: JSON.stringify(body) });
      toast.success(okMsg);
      mutate();
    } catch (e) {
  ...
  async function clearChannel(target: "telegram" | "webhook") {
    ...
    try {
      await api(`/api/admin/notify?target=${target}`, { method: "DELETE" });
      toast.success("已清除");
  ...
  async function testPush() {
    ...
    setTestResults(null);
    try {
      const r = await api<{ sent: boolean; results: TestResult[] }>(
          ...
          <div className="space-y-5">
            ...
            <div className="card p-5">
              ...
              <p className="mt-2 text-xs text-slate-500">
                每小时自动同步一次数据并扫描：机器 7/3/1 天及当天到期、进入回收站即将销毁、
                接入凭据失效同步停摆，命中即推送（同一告警只发一次，不重复轰炸）。
              </p>
            ...
            <div className="card p-5">
              ...
              <p className="mt-1 flex items-start gap-1.5 text-xs text-slate-500">
                ...
                bot token 仅服务端加密存储，绝不展示给前端或客户。配置步骤见右侧「配置攻略」。
              </p>
